...
var trackers = require("../constants/trackers")
...
function TorrentService (callback) {
  ...
  var service = {
    ...
    openTorrentUrl: function (url,trackersList) {
      ...
      if (/^magnet:/.test(url)) {
        this.onTorrent(url,trackersList)
    ...
    onTorrent: function (torrent,trackersList) {
      
      var trAry;
      if(trackersList != null){
        trAry = trackersList.split("\n\n")
        trAry = trAry.slice(0,trAry.length-2)
      }
      console.log('trAry---》'+trAry)
      var engine = peerflix(torrent, {
        tmp: _temp,
        trackers:trAry?trAry:trackers.trackers
      })
